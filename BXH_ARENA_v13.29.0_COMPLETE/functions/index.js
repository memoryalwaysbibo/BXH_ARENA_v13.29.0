...
async function awardTitle({uid, titleId, sourceType, sourceId, seasonId = null, grantedBy = "system", grantReason = ""}) {
  ...
  return db.runTransaction(async (tx) => {
    ...
    const earnedSnap = await tx.get(earnedRef);
    ...
    tx.set(defRef, {recipientCount: FieldValue.increment(1), updatedAt: FieldValue.serverTimestamp()}, {merge: true});
...
async function evaluateTitles(uid, sourceType, sourceId) {
  ...
  if (cfg.titlesEnabled !== true || cfg.automaticTitleAwardsEnabled !== true) return [];
  ...
  for (const doc of defs.docs) {
    ...
    if (currentValue >= targetValue) {
      results.push(await awardTitle({uid, titleId: doc.id, sourceType, sourceId, grantedBy: "system"}));
    }
  }
  return results;
}
...
exports.dailyCheckIn = onCall({region: REGION, cors: true}, async (request) => {
  ...
  if (!result.alreadyCheckedIn && actor.data.role !== "tester") {
    await evaluateTitles(actor.uid, "daily_checkin", today);
  }
  return {ok: true, alreadyCheckedIn: result.alreadyCheckedIn, today, stats: serializeValue(result.stats)};
});
...
exports.onLadderTransactionCreated = onDocumentCreated({document: "ladderTransactions/{logId}", region: REGION}, async (event) => {
  ...
  if (!log || log.type !== "event" || !log.playerUid || !log.eventCode) return;
  ...
  if (tour.eventAuthority !== "official" || tour.ladderMode !== "ranked" || tour.testMode === true || tour.eventCancelled === true || tour.ladderPointsAwarded !== true) return;
  ...
  if (created) await evaluateTitles(uid, "official_tournament", eventCode);
});
...
exports.grantTitle = onCall({region: REGION, cors: true}, async (request) => {
  ...
  if (!uid || !titleId || reason.length < 2) throw new HttpsError("invalid-argument", "target-title-reason-required");
...
exports.revokeTitle = onCall({region: REGION, cors: true}, async (request) => {
  ...
  const result = await db.runTransaction(async (tx) => {
    ...
    tx.create(auditRef, {action: "revoke", titleId, targetUid: uid, operatorUid: actor.uid, reason, createdAt: FieldValue.serverTimestamp()});
